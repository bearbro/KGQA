def get_url():
    ...
    # url = {"host": '42.192.36.128', "port": 6692, "timeout": 1500}#外网
    return url
'''
预测标注中抽取实体提及，并评估
'''
...
def evaluate_entity_linking(true_entity_path,pred_entity_path,no_right_linking_file=None):
    ...
    return len(no_right_link),all_right/all_true,all_line_right/766,all_pred/766
...
for idx,model_path_i in enumerate(model_path_set):
    ...
    no_n,r,line_right_r,avg_n=evaluate_entity_linking(clear_test,outfile,no_right_linking_file)
    log_wf.write('entity_linking:\tno_n:%d\tR:%.4f\tline_right_r:%.4f\tavg_n:%.4f\n'%(no_n,r,line_right_r,avg_n))
    tag+='entity_linking:\tno_n:%d\tR:%.4f\tline_right_r:%.4f\tavg_n:%.4f'%(no_n,r,line_right_r,avg_n)
    log_wf.write('tag:'+tag+'\n\n')
