...
model_dir_Path = './ckpt'
result_dir_Path = './ckpt_result'
error_dir_Path='./ckpt_error'
...
model_path_set=os.listdir(model_dir_Path)
for idx,model_path_i in enumerate(model_path_set):
    print('-'*30)
    print('%d / %d'%(idx,len(model_path_set)))
    if 'sig_acc' not in model_path_i:
        continue
    # if ',0.95' not in model_path_i:
    #     continue
    def get_args_from_name(model_file_name):
        args_str=re.sub(r'sig_acc.*', "sig_acc", model_file_name)
        args_str=args_str.split(',')
        for i in range(len(args_str)-4):
            args_str[i]=eval(args_str[i])
        return args_str
    print(model_path_i)
    args_i=get_args_from_name(model_path_i)
    ...
    def evaluate(dev_data, batch=1,tag=None):
        ...
        acc=1e-10
        if tag != None:
            error_f=open(os.path.join(error_dir_Path,'%s.txt'%tag), 'w', encoding='utf-8')
    ...
    model_path = os.path.join(model_save_path, model_path_i)
    ...
    print('val')
    score=evaluate(dev_, batch=1,tag=','.join(map(str, args_i))+'-dev')
    print("valid evluation:", 'all_acc,f1,sig_acc',score)

    print('test')
    test_ = [(test_data[i][1], test_label[i]) for i in range(len(test_data))]
    t_score = evaluate(test_, batch=1, tag=','.join(map(str, args_i))+'-test')
    result_path_i=model_path_i.replace('.weights','(%.4f,%.4f,%.4f).txt'%t_score)
    result_path = os.path.join(result_dir_Path, result_path_i)
