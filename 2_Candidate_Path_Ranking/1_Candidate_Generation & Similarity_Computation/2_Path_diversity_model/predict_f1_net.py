...
print(tag)
#model_tag='ckpt_similarity_bert_wwm_ext_f1_net_100,1e-05,8,val_monitor_f1-2020-10-27-08-05-20.hdf5'
print(model_tag)
class Config:
...
def get_Graph():
    ...
    #graph = Graph("http://47.114.86.211:57474", username='neo4j', password='pass')# 外网
    return graph
...
def _get_next_hop_path_two_old(graph,path):
    ...
    if  path_list[0]=='?x':
        ...
        cypher3=cypher+"(x)-[rel1:Relation{name:'"+path_list[1]+"'}]->(:Entity{name:'"+path_list[2]+"'}) match (y)-[rel2]->(x) return distinct rel2.name,y.name,x.name"
        # print(cypher3)
        ...
        cypher4 = cypher + "(x)-[rel1:Relation{name:'" + path_list[1] + "'}]->(ent1:Entity{name:'" + path_list[2] + "'}) match (x)-[rel2]->(y) where ent1.name<>y.name return distinct rel2.name,y.name,x.name"
        # print(cypher4)
