...
sys.path.insert(0,'/home/hbxiong/QA2/rel_and_path_similarity')#lab
...
def get_Graph():
    # graph = Graph("http://localhost:7474", username='neo4j', password='pass')# 本地
    graph = Graph("http://59.78.194.63:37474", username='neo4j', password='pass')# 校园网
    return graph
...
def get_one_ent_one_hop(graph, ent):
    ...
    '''
    # ent->rel->?x
    cypher1 = "MATCH (ent1:Entity{name:'" + ent + "'})-[rel]->(x) where rel.name<>'<类型>' RETURN DISTINCT rel.name"
    try:
...
def _get_next_hop_path_two_old(graph, path):
    ...
    sample = []
    if path_list[0] == '?x':
        cypher1 = cypher + "(y)-[rel1:Relation{name:'" + path_list[1] + "'}]->(ent1:Entity{name:'" + path_list[
            2] + "'}) match (y)-[rel2]->(x) where rel2.name<>'<类型>' and ent1.name<>x.name return distinct rel2.name"
        # print(cypher1)
        ...
        cypher2 = cypher + "(y)-[rel1:Relation{name:'" + path_list[1] + "'}]->(ent1:Entity{name:'" + path_list[
            2] + "'}) match (x)-[rel2]->(y) return distinct rel2.name"
        # print(cypher2)
        ...
        cypher3 = cypher + "(x)-[rel1:Relation{name:'" + path_list[1] + "'}]->(:Entity{name:'" + path_list[
            2] + "'}) match (y)-[rel2]->(x) return distinct rel2.name,y.name,x.name"
        # print(cypher3)
        ...
        cypher4 = cypher + "(x)-[rel1:Relation{name:'" + path_list[1] + "'}]->(ent1:Entity{name:'" + path_list[
            2] + "'}) match (x)-[rel2]->(y) where ent1.name<>y.name return distinct rel2.name,y.name,x.name"
        # print(cypher4)
    ...
    if path_list[2] == '?x':
        cypher5 = cypher + "(:Entity{name:'" + path_list[0] + "'})-[rel1:Relation{name:'" + path_list[
            1] + "'}]->(y) match (y)-[rel2]->(x) where rel2.name<>'<类型>' return distinct rel2.name"
        # print(cypher5)
        ...
        cypher6 = cypher + "(ent1:Entity{name:'" + path_list[0] + "'})-[rel1:Relation{name:'" + path_list[
            1] + "'}]->(y) match (x)-[rel2]->(y) where ent1.name<>x.name return distinct rel2.name"
        # print(cypher6)
        ...
        if path_list[1] != '<国籍>' and path_list[1] != '<类型>' and path_list[1] != '<性别>':
            cypher7 = cypher + "(ent1:Entity{name:'" + path_list[0] + "'})-[rel1:Relation{name:'" + path_list[
                1] + "'}]->(x) match (y)-[rel2]->(x) where ent1.name<>y.name return distinct rel2.name,y.name,x.name"
            # print(cypher7)
            ...
            cypher8 = cypher + "(:Entity{name:'" + path_list[0] + "'})-[rel1:Relation{name:'" + path_list[
                1] + "'}]->(x) match (x)-[rel2]->(y) return distinct rel2.name,y.name,x.name"
            # print(cypher8)
...
def _get_next_hop_path_two(graph, path):
    ...
    if path_list[0] == '?x':
        ...
        cypher3 = cypher + "(x)-[rel1:Relation{name:'" + path_list[1] + "'}]->(:Entity{name:'" + path_list[
            2] + "'}) match (y)-[rel2]->(x) return distinct rel2.name,y.name"
        # print(cypher3)
        ...
        cypher4 = cypher + "(x)-[rel1:Relation{name:'" + path_list[1] + "'}]->(ent1:Entity{name:'" + path_list[
            2] + "'}) match (x)-[rel2]->(y) where ent1.name<>y.name return distinct rel2.name,y.name"
        # print(cypher4)
        if len(answers) < 1000:
            answers = graph.run(cypher4).data()
            for ans in answers:
    ...
    if path_list[2] == '?x':
        ...
        if path_list[1] != '<国籍>' and path_list[1] != '<类型>' and path_list[1] != '<性别>':
            cypher7 = cypher + "(ent1:Entity{name:'" + path_list[0] + "'})-[rel1:Relation{name:'" + path_list[
                1] + "'}]->(x) match (y)-[rel2]->(x) where ent1.name<>y.name return distinct rel2.name,y.name"
            # print(cypher7)
            answers = graph.run(cypher7).data()
            if len(answers) <= 1000:
                for ans in answers:
            ...
            cypher8 = cypher + "(:Entity{name:'" + path_list[0] + "'})-[rel1:Relation{name:'" + path_list[
                1] + "'}]->(x) match (x)-[rel2]->(y) return distinct rel2.name,y.name"
            # print(cypher8)
            answers = graph.run(cypher8).data()
            if len(answers) <= 1000:
                for ans in answers:
